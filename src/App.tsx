...
import { CompletionGuide, Glossary } from "./components/Glossary";
...
import type { SoftWeights, SolveResult, SolverMode, TimetableInput, ValidationIssue } from "./domain/types";
...
export default function App() {
  ...
  const [notice, setNotice] = useState("دادهٔ نمونه آمادهٔ ویرایش است. برای هر فیلد راهنمای کوتاه و مثال در دسترس است.");
  ...
  return <div className="app-shell">
    ...
    <header className="topbar">
      ...
      <nav aria-label="ناوبری اصلی"><a href="#workflow">ورود داده</a><a href="#completion-guide">راهنمای تکمیل</a><a href="#glossary">تعاریف</a><a href="#methods">روش حل</a><a href="#privacy">حریم خصوصی</a></nav>
      <a className="source-link" href="https://github.com/Hhhkarimi/darschin" target="_blank" rel="noopener noreferrer">کد منبع</a>
    </header>

    <main id="main-content">
      <section className="hero">
        <div><span className="eyebrow">برای مدیران گروه دانشگاهی</span><h1>برنامهٔ هفتگی را با ورودی روشن و نتیجهٔ مدیریتی بسازید.</h1><p>فرم‌ها به‌جای شناسه‌های فنی از انتخاب‌گرهای فارسی، مثال‌های واقعی و توضیح پیامد هر قاعده استفاده می‌کنند. پس از حل نیز پوشش جلسات، بار روزها، استفاده از ظرفیت اتاق‌ها و عوامل اصلی جریمه در داشبورد نمایش داده می‌شود.</p><div className="hero-actions"><a className="primary-link" href="#workflow">شروع ورود هدایت‌شده</a><button type="button" className="secondary-button" onClick={downloadExcelTemplate}>دریافت Excel نمونه</button><a className="secondary-button" href="#glossary">دیدن تعاریف</a></div></div>
        <aside className="trust-card"><strong>چه چیزی برای مدیر گروه شفاف شده است؟</strong><ul><li>هر ورودی همراه با کاربرد و مثال</li><li>تفاوت روشن قید سخت و ترجیح نرم</li><li>حذف امن همهٔ داده‌های دستی</li><li>داشبورد شاخص‌ها و نمودارهای نتیجه</li><li>«بهینه» فقط برای وضعیت OPTIMAL</li></ul></aside>
      </section>
      ...
      <section id="workflow" className="workflow-shell">
        ...
        <div className="workflow-content">
          ...
          {step === "review" && <ReviewStep input={input} errors={errors} warnings={warnings} solverMode={solverMode} setSolverMode={setSolverMode} solving={solving} run={run} cancel={() => solveController.current?.abort()} onGoToStep={setStep} />}
      ...
      <CompletionGuide />
      <Glossary />
      <section id="methods" className="public-section"><header><span>روش حل</span><h2>دو انتخاب با محدودیت‌های شفاف</h2></header><div className="method-grid"><article><h3>روش سریع مرورگری</h3><p>داده از دستگاه خارج نمی‌شود. جست‌وجوی ابتکاری برای پاسخ سریع مناسب است، اما بهینگی سراسری را اثبات نمی‌کند.</p></article><article><h3>روش دقیق CP-SAT</h3><p>برای حداکثر ۴۰ گروه درسی، داده به‌طور موقت به تابع Python ارسال می‌شود. نتیجه فقط در وضعیت OPTIMAL «بهینهٔ اثبات‌شده» است.</p></article></div></section>
      <section id="methodology" className="public-section prose"><header><span>روش‌شناسی و محدودیت‌ها</span><h2>درس‌چین چه چیزی را بررسی می‌کند؟</h2></header><p>قیود سخت پشتیبانی‌شده شامل تداخل استاد، گروه دانشجویی و اتاق، ظرفیت، نوع فضا، تجهیزات الزامی، تعطیلی، زمان ممنوع، زمان و اتاق ثابت، تعارض سخت، الگوی زوج و فرد و ممنوعیت جابه‌جایی پشت‌سرهم میان ساختمان‌های متفاوت است. ترجیحات نرم از طریق تابع جریمه رتبه‌بندی می‌شوند.</p><p>برنامهٔ کامل یا ناقص پس از حل دوباره ارزیابی می‌شود. تشخیص تخصیص‌نیافتن، توضیح کمکی است و لزوماً اثبات کامل همهٔ علت‌های ناممکن‌بودن نیست.</p></section>
      <section id="privacy" className="public-section prose"><header><span>حریم خصوصی و امنیت</span><h2>داده کجا پردازش می‌شود؟</h2></header><p>روش سریع تمام پردازش را در مرورگر انجام می‌دهد. روش دقیق، ورودی جاری را برای همان درخواست به تابع حل می‌فرستد. برنامه عمداً آن را در پایگاه داده، localStorage، URL یا سرویس تحلیل‌گر ذخیره نمی‌کند؛ بااین‌حال امنیت دستگاه، مرورگر و زیرساخت میزبان خارج از تضمین مطلق این برنامه است.</p></section>
    </main>

    {jsonOpen && <div className="dialog-backdrop" role="presentation"><section className="dialog" role="dialog" aria-modal="true" aria-labelledby="json-title" onKeyDown={(event) => trapDialogFocus(event, closeJson)}><header><h2 id="json-title">ورود و خروج JSON</h2><button type="button" aria-label="بستن" onClick={closeJson}>×</button></header><p>JSON مسیر فنی و ثانویه است. برای ورود معمول مدیر گروه، فرم هدایت‌شده یا Excel نمونه پیشنهاد می‌شود.</p><textarea ref={jsonTextArea} aria-label="متن JSON" dir="ltr" value={jsonText} onChange={(event) => setJsonText(event.target.value)} /><div className="dialog-actions"><button type="button" onClick={() => downloadJson(input)}>دریافت JSON جاری</button><button type="button" className="primary-small" onClick={importJson}>خواندن و اعتبارسنجی</button></div></section></div>}

    <footer className="site-footer"><div><strong>درس‌چین</strong><span>ابزار متن‌باز برنامه‌ریزی دروس دانشگاهی</span></div><nav aria-label="پیوندهای پایانی"><a href="https://github.com/Hhhkarimi/darschin" target="_blank" rel="noopener noreferrer">کد منبع</a><a href="#glossary">تعاریف</a><a href="#methodology">محدودیت‌ها</a><a href="#privacy">حریم خصوصی</a></nav><a className="creator-credit" href="https://www.linkedin.com/in/hossein-karimi-8a452153/" target="_blank" rel="noopener noreferrer">کاری از حسین کریمی</a></footer>
  </div>;
}

function ReviewStep({ input, errors, warnings, solverMode, setSolverMode, solving, run, cancel, onGoToStep }: { input: TimetableInput; errors: ValidationIssue[]; warnings: ValidationIssue[]; solverMode: SolverMode; setSolverMode: (mode: SolverMode) => void; solving: boolean; run: () => void; cancel: () => void; onGoToStep: (step: Step) => void }) {
  const exactDisabled = input.courses.length > input.settings.exactCourseLimit;
  const groups = (["courses", "resources", "rules"] as Step[]).map((step) => ({ step, issues: [...errors, ...warnings].filter((issue) => issueStep(issue) === step) }));
  return <section className="editor-panel" aria-labelledby="review-title"><header className="section-title"><div><span>مرحله ۴</span><h2 id="review-title">بازبینی پیش از حل</h2></div><p>هر خطا به مرحلهٔ مربوط وصل شده است. خطا مانع حل است؛ هشدار فقط نیازمند بررسی مدیریتی است.</p></header>
    <div id="validation-summary" className={`validation-summary ${errors.length ? "has-errors" : "is-valid"}`} tabIndex={-1}><strong>{errors.length ? `${errors.length.toLocaleString("fa-IR")} خطای مانع حل` : "دادهٔ ورودی برای حل معتبر است"}</strong><span>{warnings.length.toLocaleString("fa-IR")} هشدار نیازمند بازبینی</span></div>
    <div className="review-issue-groups">{groups.map((group) => <section key={group.step}><header><h3>{stepLabel(group.step)}</h3><button type="button" onClick={() => onGoToStep(group.step)}>رفتن به این مرحله</button></header>{group.issues.length ? <ul>{group.issues.map((issue) => <li className={issue.severity === "error" ? "review-error" : "review-warning"} key={`${issue.path}-${issue.message}`}><b>{issue.severity === "error" ? "خطا" : "هشدار"}</b>{issue.message}</li>)}</ul> : <p className="review-ok">موردی برای اصلاح گزارش نشده است.</p>}</section>)}</div>
    <div className="review-summary"><div><span>گروه‌های درسی</span><strong>{input.courses.length.toLocaleString("fa-IR")}</strong></div><div><span>جلسات</span><strong>{input.courses.reduce((sum, course) => sum + course.sessions.length, 0).toLocaleString("fa-IR")}</strong></div><div><span>جلسات اجباری</span><strong>{input.courses.flatMap((course) => course.sessions).filter((session) => session.required).length.toLocaleString("fa-IR")}</strong></div><div><span>تعارض‌ها</span><strong>{input.conflicts.length.toLocaleString("fa-IR")}</strong></div><div><span>تعطیلی‌های بازه‌ای</span><strong>{input.closedSlots.length.toLocaleString("fa-IR")}</strong></div></div>
    <div className="solver-choice"><button type="button" className={solverMode === "fast" ? "selected" : ""} aria-pressed={solverMode === "fast"} onClick={() => setSolverMode("fast")}><b>روش سریع</b><span>داخل مرورگر، بدون ارسال داده</span><small>بهینگی اثبات نمی‌شود؛ برای آزمون و اصلاح سریع مناسب است.</small></button><button type="button" disabled={exactDisabled} className={solverMode === "exact" ? "selected" : ""} aria-pressed={solverMode === "exact"} onClick={() => setSolverMode("exact")}><b>روش دقیق CP-SAT</b><span>ارسال موقت برای حل، حداکثر پنج دقیقه</span><small>{exactDisabled ? `برای بیش از ${input.settings.exactCourseLimit} گروه درسی غیرفعال است` : "فقط OPTIMAL بهینهٔ اثبات‌شده است؛ FEASIBLE فقط پاسخ معتبر است."}</small></button></div>
    <div className="solve-actions"><button type="button" className="solve-button" disabled={solving || errors.length > 0} onClick={run}>{solving ? "در حال حل…" : "حل و ساخت داشبورد نتیجه"}</button>{solving && <button type="button" className="danger-button" onClick={cancel}>توقف حل</button>}</div>
  </section>;
}
...
function ResultStep({ input, result, setResult, resultHeading }: { input: TimetableInput; result: SolveResult; setResult: (next: SolveResult) => void; resultHeading: React.RefObject<HTMLHeadingElement | null> }) {
  ...
  return <section aria-labelledby="result-heading"><h2 id="result-heading" className="sr-focus-heading" tabIndex={-1} ref={resultHeading}>نتیجهٔ حل برنامه</h2><div className="result-actions"><button type="button" onClick={() => exportWithWarning("csv")}>دریافت CSV</button><button type="button" onClick={() => exportWithWarning("excel")}>دریافت Excel چندبرگه</button><button type="button" onClick={() => downloadJson(input)}>دریافت JSON ورودی</button></div>
    <ResultDashboard input={input} result={result} />
    <div className="quality-grid"><Metric label="جلسات تخصیص‌یافته" value={result.schedule.length} /><Metric label="جلسات تخصیص‌نیافته" value={result.unscheduled.length} /><Metric label="نقض سخت" value={result.hardViolations.length} /><Metric label="تابع جریمه" value={result.objective} /><Metric label="زمان حل (ms)" value={result.durationMs} />{result.bestBound !== undefined && <Metric label="بهترین کران" value={result.bestBound} />}</div>
    {result.solverStatus && <p className="solver-status">وضعیت CP-SAT: <b dir="ltr">{result.solverStatus}</b></p>}{result.diagnostics.map((item) => <p className="diagnostic-note" key={item}>{item}</p>)}
    {(result.unscheduled.length > 0 || result.hardViolations.length > 0 || result.validationErrors.length > 0) && <section className="diagnostics" aria-labelledby="diagnostics-title"><h3 id="diagnostics-title">گزارش خطا و تخصیص‌نیافتن</h3>{result.validationErrors.map((item) => <p key={item}>{item}</p>)}{result.hardViolations.map((item) => <p className="hard-error" key={item}>نقض سخت: {item}</p>)}{result.unscheduled.map((item) => <details key={item.sessionId}><summary>{item.required ? "اجباری — " : ""}{item.label}</summary><ul>{item.reasons.map((reason) => <li key={reason}>{reason}</li>)}</ul></details>)}</section>}
    <ScheduleViews input={input} result={result} onResultChange={setResult} />
    <section className="penalty-card"><h3>تفکیک جریمه‌ها</h3><p className="help-text">عدد هر مورد تعداد رخداد است؛ داشبورد بالا سهم وزن‌دار هر عامل را نشان می‌دهد.</p><div>{Object.entries(result.breakdown).map(([key, value]) => <span key={key}>{PENALTY_LABELS[key as keyof SoftWeights]}: {value.toLocaleString("fa-IR")}</span>)}</div></section>
  </section>;
}

function issueStep(issue: ValidationIssue): Step {
  if (/^(courses|retiredGroupNumbers)/.test(issue.path)) return "courses";
  if (/^(instructors|studentGroups|rooms|equipment)/.test(issue.path)) return "resources";
  return "rules";
}
...
function stepLabel(step: Step) { return ({ courses: "دروس و جلسات", resources: "منابع و فضاها", rules: "تقویم و قواعد", review: "بازبینی و حل", result: "نتیجه و خروجی" })[step]; }
function resultNotice(result: SolveResult): string { if (result.status === "optimal") return "حل دقیق با بهینگی اثبات‌شده پایان یافت؛ داشبورد مدیریتی آماده است."; if (result.status === "fallback") return `روش سریع پس از توقف روش دقیق اجرا شد. ${result.fallbackReason ?? ""}`; if (result.status === "failed-required") return "برنامه ناقص است و حداقل یک جلسهٔ اجباری تخصیص نیافته است."; if (result.unscheduled.length) return "برنامهٔ ناقص تولید شد؛ جلسات تخصیص‌نیافته و نکات قابل اقدام را بررسی کنید."; return "برنامهٔ معتبر تولید شد؛ داشبورد مدیریتی و نمودارها آمادهٔ بازبینی‌اند."; }
